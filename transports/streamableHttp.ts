...
// Apply API key middleware to all MCP routes
...
const server = app.listen(PORT, () => {
  ...
  if (API_KEY) {
    console.error(
      `API Key authentication enabled (set via MCP_API_KEY env var)`
    );
  } else {
    console.error(
      `WARNING: No API key configured. Set MCP_API_KEY env var to enable authentication.`
    );
  }
});
